...
def update_main(attr, old, new):
    senti = radio_button_group_source.active
    senti_labels=['from_csv','from_email']
    global reviews_ms1
    
    if(senti_labels[senti]=='from_csv'):
        ...
        #main_doc.clear()
        #main_doc.add_root(layout_init)
        sys.path.insert(0,'/home/gpu1/work_space/disk3_work_space3/CRM_Topic_models/CRM_bokeh_app')
        import email_source_analysis
        reviews_ms1 = email_source_analysis.main()
        #reviews_ms1 = email_source_analysis.df
        reviews_ms1 = reviews_ms1[['comments','title']]
        reviews_ms1['comments'] = reviews_ms1.comments.astype('str')
        reviews_ms1['title'] = reviews_ms1.title.astype('str')
    ...
    main_doc.clear()
    controls = column([ticker0])#, width=200)
    layout = row(controls)#, create_figure())
    layout = column([layout_init,layout])
...
radio_button_group_source = RadioButtonGroup(labels=['from_csv','from_email'], active=0)
radio_button_group_source.on_change('active', update_main)

## bokeh root init
main_doc = curdoc()

layout_init = row([radio_button_group_source])#, create_figure())
